...
public class CommentLikeService {
    ...
    public void pressLike(CommentLikeCreate commentLikeCreate){
        User user = userRepository.findById(commentLikeCreate.getUser_id()).orElseThrow(() -> new UserNotFound(UserErrorCode.USER_NOT_FOUND));
        Comment comment = commentRepository.findById(commentLikeCreate.getComment_id()).orElseThrow(() -> new CommentNotFound(CommentErrorCode.COMMENT_NOT_FOUND));
